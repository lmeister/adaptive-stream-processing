...
followed by a brief explanation of the MAPE-K loop in \ref{sub:mape} to conclude this chapter.
    ...
    \quad Architects are faced with a number of decisions;\\
    ...
    TODO: Hier den Absatz zu Ende bringen irgendwie


    % Self adaptivitiy however challenging from a software engineering perspective
    % Requirements engineering has to deal with uncertainty, due to possible new requirements b/c of adaptations during runtime
    % -> Requirements must be formulated in another way (Cant be as strict anymore)
    % -> Requirements must contain constraints for adaptations
    % -> Traceability from requirements to implementation a lot harder, since requirements change during runtime
    % -> RE has to define what exactly has to be monitored (Check this one again)
    % -> RE has to define a set of high-level goals that must always be maintained, no matter the adaptation
    % For architecture and engineering:
    % -> Centralized MAPE loop has bad scaling
    % TODO: Ab hier weiter machen und so
    % -> frequent adaptations are bad, b/c of large overhead
    %     -> Possible solution would be feedback based (e.g. reinforcement learning, already a few implementations, see 2nd paper)
    % -> Hard to manage multiple MAPE loops, which are good though, especially if independent
