...
\textbf{Add and explain a few more MAPE Based architectures}
    ...
    NOTE: Explain what Dhalion is, where its used
        ...
        % Dhalion sits on top of streaming engines and allows them to become self-regulating/adapting
        % 3 major components:
        
        % Health Manager
        %   -> Mainting the health of the running topology
        %   -> periodically invokes a policy which evaluates the status/health of topology
        %   -> policies can be either invasive or noninvasive
        %       -> invasive: takes action that adjust topology config, such as parallelism changes
        %       -> noninvasive: No topology changes, typically a alert to the user when particular event happens
        %   -> Can execute multiple noninvasive policies at a time, but only one invasive
        %       -> So that no conflicting actions are taken, as it would cause instability 
        %   -> 3 phases:
        %       -> Symptom Detection Phase
        %       >>>> Dhalion collects several metrics during this phase
        %       >>>> Identify symptoms that indicate that health of topology is compromised
        %       >>>> (Backpressure (nachlesen), skew)
        %       >>>> Identification of symptons done by Symptom Detectors, that collect appropriate metrics and form evaluations of said metrics
        %       >>>> Symptom Detectors employ anomaly detection techniques, outliers, clustering..
        %       >>>> Once a symptom has been identified, a Symptom description is produced, containing the symptom, the metrics and their corresponding values used to identify the symptom
        %       >>>> API so that users can create their own Symptom Detectors
        
        %       -> Diagnosis Generation Phase 
        %       >>>> Collects the symptoms generated by Sympton Detection Phase
        %       >>>> Attempts to identify root cause of symptoms
        %       >>>> Done via Diagnosers, who take a set of symptom descriptions of input
        %       >>>> output is a diagnosis description based on symptoms, if possible
        %       >>>> Diagnosis Description is respresentation of root cause, symptoms and their metrics + values that led to this diagnosis
        %       >>>> API to create new Diagnosers

        %       -> Resolution Phase
        %       >>>> Resolve problems identified by Diagnosis Generation Phase
        %       >>>> Done by Resolvers
        %       >>>> Resolvers take asinput a diagnosis description
        %       >>>> based on diagnosis description performs appropriate action to bring topology back to healthy state
        %       >>>> Usually Diagnosers 1 : 1 Resolvers
        %       >>>> API as well
        %       >>>> First step: Determine appropriate Resolver by examining diagnoses
        %       >>>> Second step: Resolver perfoms the topology changes
        %       >>>> However, major topology changes are invoked through scheduler (e.g. scaling up/down, restarting container)
        
        % Action Log
        %   -> captures the actions taken by Health Manager
        %   -> useful for tuning policies and debugging
        %   -> each entry contains what action was taken, at what time, and which diagnosis
        %   -> Configurable how many logs, how old .. etc kept
        
        % Action Blacklist
        %   -> Diagnosis Description + actions tken that did not produce expected outcome
        %   -> These actions will not be invoked again for a similar diagnosis
        %   -> After each Action the Health Manager will wait, allow topology to reach a steady state and then evalute action
        %   -> When new policy is created, evaluation method must be defined
        %   -> Threshold for blacklisting actions. If particular action has not been beneficial for given diagnosis
        %   -> Before invoking an action, Dhalion checks the blacklist
        %   -> Action Blacklist is optional for policies
        ...
        % Highly customizable as one can create own policies and great API (Diagnosers, symptom checkers, resolvers)!!!
        % -> Modularität
    ...
    % \\
    In \ref{sec:edf} we will present the proposed architecture, as shown in figure \ref{fig:hierarchical}, and elaborate on its functionality in greater detail.
    % \\
    Afterwards we will discuss the architecture, talk about the advantages and possible complications. Further we will compare it to its reference architecture in \ref{sec:discussion-edf}.
        ...
        % recht general, nicht zu restrictive, viele policies moeglich?
