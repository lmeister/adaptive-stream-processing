...
explain the proposed architecture in \ref{sub:edf} and follow it up with a discussion in \ref{sub:discussion-edf}.
    ...
    followed by a discussion of Dhalion in \ref{sub:dhalion-discussion}.
        ...
        \quad However, 
        % -> Probleme, wenn während Resting period für evaluation ein anderes Problem auftritt?
        % -> 
        % -> Vergleich MAPE? TODO
        % M: Health Manager Sympton Detector (Collects metrics)
        % A: Health Manager Symptom Detector (Analyzes metrics, identifies symptoms) & Diagnoser (Forms diagnosis)
        % P: Resolver (Determines necessary action)
        % E: Resolver executes determined action
    ...
    Afterwards we will discuss the architecture, talk about the advantages and possible complications. Further we will compare it to its reference architecture in \ref{sub:discussion-edf}.
        ...
        %   ->  As previously explained even a single centralized component can make controlling elasticity unfeasible
