...
followed by a brief explanation of the MAPE-K loop in \ref{sub:mape} to conclude this chapter.
    ...
    some requirements that SPSs should meet.
        ...
        \quad Secondly, DSMSs operate at a much greater speed than traditional DBMSs, because the data is processed immediately, while in the DBMS approach
        ...
        \quad Thirdly, similar to a DBMS a DSMS can also be queried using a streaming query language, however, queries installed on a DSMS are continuous 
        ...
        % Big data erklären??
        % Wie sieht Data aus in SPS? Tupel erläutern (vergleich obj), infinite, structure (unstructured, semi-, structured)
        % Data sinks, data generators? Terminologie klären
        % Made up of operators?, Operators funktionen erklären 
        % Erklärung von Windows irgendwie einbringen (Hier noch überlegen wo am besten)
        % Erklären wozu SPS eingesetzt
        % Erklärung Adaption, wieviele Arten von Adapation, welche Arten
        ...
        \quad A data tuple is an object whose internal structure is defined by a \texit{data scheme}. 
        ...
        \quad The results of a streaming application is then fed into one or multiple \textit{data sinks}, which could for example be a data visualization engine 
        ...
        There are many strategies of adaptation, as listed by Qin et al in \cite[p.8f]{QIN20191}. An examplary adaptation would be a processing scaling adaptation,
        as shown in figure \ref{fig:sps_parallel_normal}, where, if computational resources allow it, an operator may be replicated in order to process data faster by
        making use of the principle of parallelity. Technically, there should be a splitting operator placed before the replicated node, as well as a merging operator
        after the replicated note. These have been left out for the sake of brevity.
